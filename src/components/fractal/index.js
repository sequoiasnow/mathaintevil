...
class Fractal extends React.Component {
  ...
  renderBranch(x, y, theta, scale, n) {
    ...
    return  (
      <g transform={`translate(${110} ${-24 * 0.35}) scale(${nscale}) rotate(-20)`}>
        <circle cx="0" cy="0" fill="red" r="40" />
        <use href="#branch" />
        <g transform={`translate(${-154} ${-70}) rotate(-10)`}>
          {this.renderBranch(x, y, 0, scale, n + 1)}
        </g>
        {this.renderBranch(x, y, 0, scale, n + 1)}
        <g transform={`translate(${344} ${20}) rotate(120)`}>
          {this.renderBranch(x, y, 0, scale, n + 1)}
        </g>
      </g>
    )
